...
const UserLogPage = () => {
  ...
  const handleDelete = (logId) => {
    ...
    const updatedLogs = logs.filter(log => log.userId !== logId);
    
    // Update state
    setLogs(updatedLogs);
    setFilteredLogs(filteredLogs.filter(log => log.userId !== logId));
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredLogs.length === 0 ? (
                  ...
                  filteredLogs.map((log) => (
                    <tr key={log.userId} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        {deleteConfirm === log.userId ? (
                          <div className="flex justify-end space-x-2">
                            <button
                              onClick={() => handleDelete(log.userId)}
